...
> **한국은행 ECOS API → 32개 지표 수집 → 10개 파생 신호 → 2×2 레짐 분류 → claude.ai 자동 분석**
...
| 02 | 물가·인플레 | 4 | CPI, 근원CPI(농산물·석유류 제외, QB), PPI, 수입물가지수(403Y005/B) |
| 03 | GDP·경기·생산 | 5 | 실질GDP(QoQ/YoY), 경기동행지수, 경기선행지수, 광공업생산 YoY |
| 04 | 노동시장 | 4 | 실업률, 취업자수증감, 경제활동참가율, 고용률 |
| 05 | 통화·유동성 | 3 | M2 광의통화 YoY(161Y006), 본원통화, 예금은행 총대출금(104Y016) |
| 06 | 주택시장 | 3 | KB주택매매가격 YoY(901Y062), KB전세가격 YoY(901Y063), 착공지수 |
| 07 | 수출입·무역 | 2 | 수출금액 YoY, 수입금액 YoY (금액지수, 403Y003/403Y001) |
| 08 | 금융시장 | 5 | KOSPI, KOSDAQ, 원/달러 환율(731Y004), CD-기준금리 스프레드, 크레딧스프레드 |
...
> **v2.2 수정**: (1) 근원CPI 오류항목 정정 (item "11"=신선어개 → "QB"=농산물·석유류제외), (2) 잘못된 주택 시리즈(901Y092=수출입 무역데이터) 제거 후 KB주택가격지수로 대체, (3) CORP\_LOAN(104Y014=수신합계, 오류) → BANK\_LOANS(104Y016=총대출금)으로 교체, (4) 수입금액 YoY ≥30% 시 기저효과 경고 플래그, (5) GDP 성장 점수 범위 (-3,6) → (-2,8) 클리핑 해소
...
| SIG11 | 주택시장 | KB매매가격YoY, KB전세가격YoY, 착공지수 | 매매가격 YoY >10% 과열 / 착공 <80 공급 급감 |
