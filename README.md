...
> **한국은행 ECOS API → 33개 지표 수집 → 10개 파생 신호 → 2×2 레짐 분류 → claude.ai 자동 분석**
...
| SIG06 | 소비자심리 | CSI, 소매판매 YoY | CSI <80 심각 위축 |
...
**성장 점수** (7요소): 실질GDP, 소매판매, 고용률, 경기동행/선행지수, CSI, KOSPI  
...
3. 대화 시작 시 `data/` 폴더의 4개 파일 자동 참조:
   - `ecos_latest.md` — 33개 원천 팩트 테이블
   - `ecos_signals.md` — 10개 신호 대시보드
...
1. 현재 매크로 레짐과 10개 신호의 핵심 시사점 요약
